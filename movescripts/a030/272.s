.nds
.thumb

.include "include/battlescriptcmd.s"
.include "include/abilities.s"
.include "include/itemnums.s"
.include "include/monnums.s"
.include "include/movenums.s"

.create "a030\a030_272", 0

a030_272:
    ifmonstat IF_MASK, BATTLER_ATTACKER, 0x35, 0x1000, _00C8
    if IF_MASK, VAR_06, 0x200, _00C8
    checkitemeffect 0x0, BATTLER_ATTACKER, 0x63, _0078
    changevartomonvalue VAR_OP_SETMASK, BATTLER_ATTACKER, VAR_RULESET, 0x20000000
    changevar VAR_OP_SET, VAR_STATUS1, 0x4000000C
    changevar VAR_OP_SETMASK, VAR_06, 0x23
    endscript
_0078:
    changevartomonvalue VAR_OP_SETMASK, BATTLER_ATTACKER, VAR_RULESET, 0x20000000
    changevartomonvalue VAR_OP_SETMASK, BATTLER_ATTACKER, VAR_60, 0x20000000
    gotosubscript 217
    togglevanish BATTLER_ATTACKER, 0x1
    if IF_EQUAL, VAR_DEFENDER, 0xFF, _00E0
_00C8:
    changevar VAR_OP_SET, VAR_STATUS2, 0x2000004F
    critcalc
    damagecalc
_00E0:
    changevartomonvalue VAR_OP_CLEARMASK, BATTLER_ATTACKER, VAR_RULESET, 0x200400C0
    gotosubscript 259
    endscript

.close
